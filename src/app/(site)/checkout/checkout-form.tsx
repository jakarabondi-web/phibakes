...
import type { PlatformSettingsValues, ZoneRate } from "@/lib/platform-settings";
...
export function CheckoutForm({
  ...
  // Only zones the owner has left switched on are offered to customers.
  const activeZones = zones.filter((z) => z.isActive);
  const feeForZone = (name: string | undefined | null) =>
    activeZones.find((z) => z.zone.toLowerCase() === (name ?? "").toLowerCase())?.fee ?? 0;
  const STUDIO_ADDRESS = settings.studioAddress;
  const STUDIO_HOURS = settings.studioHours;
  ...
  let deliveryFee = fulfilment === "delivery" ? feeForZone(zone) : 0;
  // Owner-configured free-delivery threshold, applied before the deposit split.
  if (
    fulfilment === "delivery" &&
    settings.freeDeliveryAbove != null &&
    subtotal - discount >= settings.freeDeliveryAbove
  ) {
    deliveryFee = 0;
  }
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <Card className="gap-5 p-6">
            <CardHeader className="p-0">
              ...
              <CardDescription>Pay a {settings.depositPercent}% deposit now, or settle in full today.</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              ...
              >
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">Pay {settings.depositPercent}% Deposit</p>
            ...
            <CardContent className="flex flex-col gap-4 p-0">
              ...
              <div className="rounded-xl bg-secondary/60 p-3 text-xs text-muted-foreground">
                <div className="flex justify-between">
                  <span>Due now ({paymentPlan === "deposit" ? `${settings.depositPercent}% deposit` : "full"})</span>
